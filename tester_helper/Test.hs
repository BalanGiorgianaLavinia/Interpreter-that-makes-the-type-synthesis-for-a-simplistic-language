-- Authors: Lucian Mogoșanu, Mihnea Muraru, Andrei Olaru.
...
--tests :: Int -> [(String -> d -> Bool)]
tests ex points testList = sequence_ $ map tt testList
      where
        total = foldl (+) 0 $ map (\(_, p, _, _) -> p) testList
        tt (s, d, e, b) = assertValIO ("[" ++ (show ex) ++ "] " ++ s) (points * d / total) e (return b)

testOne ex (s, d, e, b) = assertValIO ("[" ++ (show ex) ++ "] " ++ s) d e (return b)

test s d e b = (s, d, e, b)

testCond b s d = test s d "boolean test failed" b

testVal vg ve s d = test s d err (vg == ve)
